...
from mlflow_reports.common.click_options import opt_output_file_base, opt_get_raw
...
def list_endpoints(
    ...
    io_utils.write_csv_and_json_files(output_file_base, endpoints, columns, ts_columns, normalized_pandas_df)
...
def main(columns, output_file_base, call_databricks_model_serving, get_raw=False, normalized_pandas_df=False):
    print("Options:")
    for k,v in locals().items():
        print(f"  {k}: {v}")
    if isinstance(columns, str):
        columns = columns.split(",")
    list_endpoints(columns, output_file_base, call_databricks_model_serving, get_raw, normalized_pandas_df)
